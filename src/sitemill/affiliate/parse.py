...
_NOISE = re.compile(
    r"\d+"
    r"|[／/|｜・\-–—=＝*＊★☆▼▲><]+"
    # 画面の見出しは件数を連れていることがある（「検索結果 31件」）。
    # 全体一致で見ているので、後ろの件数まで含めて食べないと見出しとして残る
    r"|(?:検索結果|検索条件|該当件数)[\s:：]*[\d,]*\s*件?(?:中.*)?"
    r"|検索結果|検索条件|並び替え|絞り込み|新着順|人気順|おすすめ順|該当件数"
    # 並び替えの操作は選択中の値を連れている（「並び替え 新着順」）。
    # 1 行に収まっていると上の単語だけでは当たらず、案件名を 1 行押し出す
    r"|(?:並び替え|表示順|絞り込み)[\s:：]*(?:新着順|人気順|おすすめ順|報酬順|評価順)?"
...
# ラベルの無い提携状況（A8 は「未提携」、もしもは「未申請」が 1 行で出る）。
# **ここに無い語は見出しとして扱われる**。もしもの一覧は「…／提携状況／案件名」の順に並ぶので、
# 「未申請」を知らないと 31 件中 30 件の案件名が「未申請」になった（akiya-atlas の実データ）
_STATUS = re.compile(
...
# 「未申請」＝こちらがまだ申請していない。申請が要る側として数える（もしもの表記）
_NEEDS_REVIEW = re.compile(r"審査|承認制|要申請|申請が必要|申請中|未提携|未申請|提携申請")
# ASP が案件に付ける可否の札（「本人 NG」「リスティング NG」）。
# 案件名ではないので見出しにしない。見出しにすると、次の案件の名前を 1 行押し出す
_FLAG_LINE = re.compile(
    r"^(?:本人|自己|セルフバック|リスティング|メール|SNS|アダルト|ブログ|相互|クーポン)"
    r"\s*(?:購入|申込|広告)?\s*(?:NG|OK|可|不可|禁止|許可|あり|なし)$",
    re.IGNORECASE,
)
...
def _split_records(text: str) -> list[_Record]:
    ...
    while i < len(lines):
        ...
        line = lines[i]
        i += 1
        if not line or _NOISE.fullmatch(line) or _FLAG_LINE.match(line):
            continue
