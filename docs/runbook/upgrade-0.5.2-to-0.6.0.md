...
- **サイトマップの lastmod を「ページの中身が最後に変わった日」にする**（sitemill ADR 0025）。
  ...
  - **テンプレートの変更はほぼ要らない**。コミット済みの写しを 9/17 と 9/18 の日付でビルドして比べると、
    指紋が変わったのは 1 / 6,421 ページ（トップの「最終更新は 2026年…」の 1 行、
    `templates/index.html` の 104 行目あたり）。気になるなら、その `<li>` に `data-sitemill-volatile` を付ける
  - 物件ページの「取得日時」は巡回の時刻ではなく記録の抽出日なので、そのままでよい
