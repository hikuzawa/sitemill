# akiya-atlas を sitemill v0.5.2 から v0.6.0 / v0.6.1 / v0.7.0 に上げる
...
## v0.7.0 に上げる場合（2026-09-17 時点の推奨）

**v0.7.0 に直接上げてよい**。手順は下と同じで、`ref:` を `v0.7.0` にする。v0.6.1 との差は 1 つ:

- **サイトマップの lastmod を「ページの中身が最後に変わった日」にする**（sitemill ADR 0025）。
  いまの akiya-atlas は 6,421 ページ中 6,390 ページの lastmod が毎日「今日」で、Google は lastmod を
  信用しない状態にある。`sitemill build` がページの `<main>` の指紋を `data/state/lastmod.json` に残し、
  中身が変わったページだけ日付を進める
  - `data/state/lastmod.json` は日次でコミットされる（`pipeline.yml` の `git add data/state` に入っている）。
    約 6,400 件ぶんで、日次の差分は変わったページの行だけ
  - **確かめ方（2026-09-19 に直した。前の書き方は見立てを外していた）**: 同じデータを 9/17 と 9/18 の
    「日付」でビルドして比べる形では、**再取得のたびに動く「取得日時」「最終取得」の表示が見えない**
    （データが同じなら取得日時も同じなので、差が出ない）。**日付を固定して、実データを 2 日分
    入れ替えて**比べる。akiya-atlas はこの見立てのまま上げてしまい、2 回目の日次で
    972 ページが「変わった」（実質 300 件）になり、あとから 9 か所に `data-sitemill-volatile` を
    足した。手順は次のとおり。

    ```bash
    # 作業ツリーを 2 つ用意し、コードは同じコミット、data だけ 2 日分にする
    git worktree add --detach /tmp/wt_a <今のコード>
    git -C /tmp/wt_a checkout <1 日目の data コミット> -- data
    git worktree add --detach /tmp/wt_b <今のコード>
    git -C /tmp/wt_b checkout <2 日目の data コミット> -- data
    # 同じ now で両方をビルドし、sitemill.build.lastmod.fingerprint を突き合わせる
    ```

    指紋が変わったページの `<main>`（volatile を外したもの）のテキストを差分で見て、**時刻・取得日時の
    ちがいだけで変わっているページが 1 つでもあれば、その表示に `data-sitemill-volatile` を付ける**。
    japan-open-today は 892 ページ中 6 ページで、6 ページとも引用の言い直し（下の「言い回しの凍結」）。
    取得日時だけで変わったページは無かった（取得日時の表示が volatile な `.why` の中と `<main>` の外に
    あるため）
  - **言い回しの凍結（v0.7.3）**: 同じ本文でも LLM は言い直す。値が同じなら引用・見出しを前回のまま
    残す（`sitemill.store.wording.freeze_wording`）。取り込みの統合関数で
    `freeze_wording(existing[key], value)` を通す。akiya-atlas は補助制度で同じ対処を自前で入れている
    （区分・要約の凍結）。**引用そのものが値の項目（住所・電話のような文字列）は、値が変わるので
    凍結できない**。言い直しがそのまま `lastmod` に出る
  - **見た目を変えた回の台帳（v0.7.4）**: テンプレートを変えると全ページの指紋が変わる。v0.7.4 から
    エンジンが自動で作り直し、**日付は据え置く**（台帳の `layout` が変わった回。実行レポートの
    `build.lastmod_relearned` とビルドの警告に出る）。akiya-atlas の `operations.md` 3-6 の手作業は、
    v0.7.4 に上げれば要らなくなる（上げるまでは手順どおり手で作り直す）。
    **本番で確認済み**: japan-open-today がデザインを全面的に変えた翌日の日次で
    `lastmod_relearned 891` / `lastmod_changed 0`、台帳の日付の分布は前日と同一
    （867 / 18 / 6）だった（2026-09-21）
  - 物件ページの「取得日時」は記録の抽出日だが、**抽出は再取得のたびに走る**ので日付は動く。
    `<main>` の中に出しているなら印を付ける（akiya-atlas の 9 か所がこれ）
  - **上げた後に見ること**: ビルドの実行レポートの `build.lastmod_changed` / `lastmod_added` / `lastmod_kept`。
    初回は全ページが `lastmod_added`（日付は信頼シグナルの日付＝ほぼ今日）。**2 回目以降、`lastmod_changed` が
    ほぼ全ページなら、日付で変わる表示が `<main>` の中に残っている**（その場合は印を付ける）
