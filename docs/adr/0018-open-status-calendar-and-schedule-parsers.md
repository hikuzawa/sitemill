...
## 追記（2026-09-13）: 役所の窓口時間は営業時間にしない（規則として）

同じ取り違えを 3 度した。

1. S6-1: 香川県のフッターの「開庁時間 8:30〜17:15」を県立ミュージアムの開館時間として発見した
2. S6-3 ②: 観音寺市のページのフッターの「開庁時間」で粟井神社の時間が「確認済み」になった
3. 施設のページの「電話受付 9:00〜17:00」

そのつど発見側（`tools/discover_pages.py`・`tools/municipal_pages.py`）で塞いできたが、
**発見の道が増えるたびに同じ穴が開く**。値を作る手前、`parse_opening_hours` に置く。

- 「開庁時間」「執務時間」「窓口（の受付）時間」「事務所・事務局の時間」「電話でのお問い合わせ・
  受付・対応」「お問い合わせ受付」を含む断片は、時間帯として値にしない
- それしか無ければ `(None, "office_hours")` を返す。**引用は残る**ので、根拠は表示できる
- 括弧の中だけが窓口・電話の話なら、括弧を落として本体は生かす
  （「9:00〜17:00（電話でのお問い合わせは9:00〜16:00）」→ 9:00〜17:00）
- 「入館は16:30まで」は締切として従来どおり残す
